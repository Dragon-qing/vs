...
// i的有效范围[0,plist->len]
{
    // 请在这里补充代码，完成本关任务
    /********** Begin *********/
    if((i<0||i>slist->len)||SL_IsFull(slist)){
        return false;
    }
    for(int j=slist->len-1;j>=i;j--){slist->data[j+1]=slist->data[j];}
    slist->data[i]=x;
    slist->len++;
    return true;
...
// 返回被删除的数据元素的值。
{
    // 请在这里补充代码，完成本关任务
    /********** Begin *********/
    if(SL_IsEmpty(slist)||(i<0||i>=slist->len)){
        exit(1);
    }
    T re;
    re=SL_GetAt(slist,i);
...
// 存在值为x的结点则返回结点编号, 未找到返回－1
{
    // 请在这里补充代码，完成本关任务
    /********** Begin *********/
    int index=SL_FindValue(slist,x);
    if(index==-1){
        return -1;
    }else{
        SL_DelAt(slist,index);
    }
    return index;
